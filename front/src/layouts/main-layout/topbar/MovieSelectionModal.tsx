      ...
      <DialogContent>
        {loading ? (
              ...
              {recommendations.map((movie) => (
                <Grid item xs={6} sm={4} md={3} key={movie.id || movie.title}>
                  <Card
                    ...
                    sx={{
                      cursor: "pointer",
                      border: selectedRecommendation?.id === movie.id ? "3px solid #6b8dd6" : "none",
                    }}
                  >
                    <CardMedia
                      component="img"
          ...
          <>
            ...
            <Grid container spacing={2}>
              {movies.map((movie) => (
                <Grid item xs={6} sm={4} md={3} key={movie.id}>
                  <Card onClick={() => handleSelectMovie(movie)} sx={{ cursor: "pointer", border: selectedMovies.some(m => m.id === movie.id) ? "3px solid #6b8dd6" : "none" }}>
                    <CardMedia
                      component="img"
                      height="200"
                      image={movie.poster_path ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` : "/fallback-image.jpg"}
            ...
            {totalPages > 1 && <Pagination count={totalPages} page={page} onChange={(_, value) => setPage(value)} />}
          </>
        )}
      </DialogContent>
      <DialogActions>
        {recommendations ? (
          ...
          <Button onClick={handleSubmit} color="primary" variant="contained" disabled={selectedMovies.length === 0 || loading}>
